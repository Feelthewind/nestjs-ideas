...
export class IdeaService {
  ...
  private async vote(idea: IdeaEntity, user: UserEntity, vote: Votes) {
    const oppisote = vote === Votes.UP ? Votes.DOWN : Votes.UP;
    if (
      idea[oppisote].filter(voter => voter.id === user.id).length > 0 ||
      idea[vote].filter(voter => voter.id === user.id).length > 0
    ) {
      idea[oppisote] = idea[oppisote].filter(voter => voter.id !== user.id);
